...
class Warehouse:
    ...
    def offGridMove(self, newPos, oldPos, agent):
        ...
        if newPos in self.walls:
            return True
        if newPos not in agent.stateSpacePlus:
            return True
    ...
    def step(self, action, agent, otherAgentPos):
        ...
        if not self.offGridMove(resultingPos, agent.agentPosition, agent):
            ...
            return resultingState, agent.reward, agent.isTerminalState(agent.agentPosition), None
        else:
            reward = -1
            return agent.agentPosition, reward, agent.isTerminalState(agent.agentPosition), None
