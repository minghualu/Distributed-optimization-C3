    ...
    for i in range(numGames):
        ...
        while not (done[0] and done[1]):
            ...
            for agent in agents:
                ...
                pos, reward, done[j], info = env.step(action_old, j)
                observation_new[j] = agent.getState(pos, otherAgentPos)
                ...
                agent.Q[observation_old[j], action_old] = agent.Q[observation_old[j], action_old] + ALPHA * (
                        reward + GAMMA * agent.Q[observation_new[j], action_new] - agent.Q[
                    observation_old[j], action_old])
